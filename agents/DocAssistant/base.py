...
class PDFLearningAssistant:
    ...
    def __init__(self, user_id: str = "default_user"):
        ...
        self.session_id  = f"session_{datetime.now().striftime('%Y%m%d_%H%M%S')}"

        self.memory_tool = MemoryTool(user_id = user_id)
        self.rag_tool = RAGTool(ragnamespace=f"pdf_{user_id}")
    ...
    def ask(self, question: str, use_advanced_search: bool = True) -> str:
        ...
        # 【RAGTool】 获取相关问题答案
        answer = self.rag_tool.exetuce(
            "ask",
            quesiton = question,
            limit = 5,
            enabled_advanced_search = use_advanced_search,
            enabled_mqe  = use_advanced_search,
            enabled_hyde = use_advanced_search 
        )
